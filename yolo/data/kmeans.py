...
class YOLO_Kmeans:
    def __init__(self, annotations_path, input_width, input_height, cluster_number, filename):
    ...
    def txt2boxes(self):
        ...
        for idx, line in enumerate(f):
            infos = line.split(" ")
            length = len(infos)
            image_id = int(infos[0].split('/')[-1][:-4])
            imgIds = self.coco.getImgIds(imgIds=[image_id])
            img = self.coco.loadImgs(imgIds)[0]
