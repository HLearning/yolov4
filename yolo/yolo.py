...
class YOLO(object):
    ...
    def calculation_loss(self, y_true, y_pred):
        """
        loss 函数
        ...
        # 1. 处理预测数据
        pred_xywh = self.pred_box(y_pred, self.anchors, self.num_classes, calc_loss=True)

        # 3. 处理正确label
        true_xy, true_wh, true_obj, true_class_probs = tf.split(y_true, (2, 2, 1, self.num_classes), axis=-1)
        # 将xy转成偏移量
        true_xy = true_xy * grid_shape[::-1] - grid
        true_wh = tf.math.log(true_wh / self.anchors * self.input_shape[::-1])
        ...
        # 5. 计算loss
        xy_loss = box_loss_scale * true_obj * tf.nn.sigmoid_cross_entropy_with_logits(true_xy, y_pred[..., :2])
        wh_loss = box_loss_scale * true_obj * 0.5 * tf.square(true_wh - y_pred[..., 2:4])
        ...
        class_loss = true_obj * tf.nn.sigmoid_cross_entropy_with_logits(true_class_probs, y_pred[..., 5:])
    ...
    def calculation_loss(self, y_true, y_pred):
        ...
        pred_xywh = self.pred_box(
            y_pred, self.anchors, self.num_classes, calc_loss=True)
        ...
        # 将xy转成偏移量
        true_xy = true_xy * grid_shape[::-1] - grid
        true_wh = tf.math.log(true_wh / tf.cast(self.anchors, tf.float32) * tf.cast(self.input_shape[::-1], tf.float32))
        true_wh = tf.where(tf.math.is_inf(true_wh), tf.zeros_like(true_wh), true_wh)
        # 给box更高的权重
        box_loss_scale = 2 - y_true[..., 2:3] * y_true[..., 3:4]
        ...
        # return tf.cast(xy_loss + wh_loss + obj_loss + class_loss, tf.float32)